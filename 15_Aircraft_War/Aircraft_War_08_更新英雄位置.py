...
screen.blit(hero, (150, 300))  # screen对象调用blit方法，blit方法可以接收两个参数，第一个是刚刚加载的图像，第一个参数是元组，元组的第一个数值是水平方向的坐标，第二个数值是竖直方向的坐标。
...
# 1. 定义rect记录飞机的初始位置
hero_rect = pygame.Rect(150, 300, 102, 126)   # 定义变量hero_rect记录飞机初始位置，使用pygame里的Rect类，指定 x y 宽 高 四个参数 。
