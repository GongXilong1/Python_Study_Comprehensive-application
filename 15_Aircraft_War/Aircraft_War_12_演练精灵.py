import pygame  # 使用import导入模块，在导入后的下方，需要使用模块内的内容时，必须是模块名.的方式去使用
from plane_sprites import *  # 从plane_sprites模块导入所有内容。使用from导入模块，使用时可以直接使用模块提供的工具，不再需要输入模块的名称。
...
# 创建敌机的精灵
enemy = GameSprite("./images/enemy1.png")  # 敌机的精灵命名enemy，通过派生的GameSprite类创建精灵对象，第一参数需要指定图像名称。
enemy1 = GameSprite("./images/enemy1.png", 2)  # 敌机的精灵命名enemy1，通过派生的GameSprite类创建精灵对象，第一参数需要指定图像名称，第二个参数指定敌机的速度值2，

# 创建敌机的精灵组
enemy_group = pygame.sprite.Group(enemy, enemy1)  # 敌机精灵组命名为enemy_group，使用pygame提供的sprite模块中的Group类来创建一个精灵组，可以把多个精灵以多值参数的方式传递给精灵组。
...
while True:  # 创建while的无限循环，保证游戏不会立即退出，窗口一致显示。
    ...
    # update方法 - 可以让精灵组中所有的精灵对象都执行一下update方法，让组中的所有精灵都更新位置，
    enemy_group.update()  # 使用enemy_group调用update方法。
    # draw方法 - 会把精灵组中所有精灵的图像绘制在screen屏幕上
    enemy_group.draw(screen)  # 使用enemy_group调用draw方法，在调用draw方法时，需要把screen这个屏幕对象传递给draw方法，因为精灵组需要知道把精灵组内部的精灵绘制到谁的身上。
